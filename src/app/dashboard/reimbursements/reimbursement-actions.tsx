...
import { rejectReimbursement, markAsPaid } from "./actions"
...
export function ReimbursementActions({ request, onActionComplete }: { request: any, onActionComplete: () => void }) {
  const [isLoading, setIsLoading] = useState<"reject" | "pay" | null>(null)
  const { toast } = useToast()
  const { user: currentUser } = useAuth();
  
  const canManage = currentUser?.role === 'treasurer';

  const handleAction = async (actionFn: () => Promise<void>, type: "reject" | "pay") => {
    if (!currentUser) {
    ...
    try {
      await actionFn();
      toast({ title: `Request ${type === "reject" ? "Rejected" : "Paid"}` });
      onActionComplete();
    } catch (error) {
      toast({
        variant: "destructive",
    ...
    return (
      <div className="flex w-full justify-end gap-2">
        ...
        <AlertDialog>
            ...
            <AlertDialogContent>
                <AlertDialogHeader>
                    <AlertDialogTitle>Are you sure you want to mark as paid?</AlertDialogTitle>
                    <AlertDialogDescription>This will finalize the reimbursement and create financial records. This action cannot be undone.</AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                    <AlertDialogCancel>Cancel</AlertDialogCancel>
                    <AlertDialogAction onClick={onPay}>Confirm Payment</AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
      </div>
